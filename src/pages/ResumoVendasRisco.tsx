...
const emptyBucket = (): Bucket => ({
  qtd: 0, faturado: 0, recebido: 0, emAberto: 0,
...
export default function ResumoVendasRisco() {
  ...
  const carregar = async () => {
    ...
    try {
      ...
      let qv = supabase.from("vendas")
        .select("id, score, valor_total, empresa_id")
        .gte("created_at", inicio.toISOString())
        .lte("created_at", fim.toISOString());
      if (empresaId !== "todas") qv = qv.eq("empresa_id", empresaId);
      ...
      if (vendaIds.length > 0) {
        ...
        for (let i = 0; i < vendaIds.length; i += chunkSize) {
          ...
          const { data, error } = await supabase.from("parcelas")
            .select("venda_id, valor, valor_pago, status, vencimento, pago_em")
            .in("venda_id", chunk);
          if (error) throw error;
          parcelas = parcelas.concat(data ?? []);
        }
      }
  ...
  const linhas: { label: string; render: (b: Bucket, k: FaixaKey) => React.ReactNode; total: () => React.ReactNode; pctRow?: boolean }[] = [
